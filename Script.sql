...
select sum(m.id) as total_modul from modul m where m.id_guru = 1
...
select count(distinct(b.id_modul)) as total_modul from belajar b where b.id_murid = 4

select count(distinct(b.id_modul)) as total_modul from belajar b where b.id_murid = 4 and b.nilai is not null

select cast((sum(b.nilai) / count(distinct(b.id_modul))) as int) as rata_nilai from belajar b where b.id_murid = 4 and b.nilai is not null
